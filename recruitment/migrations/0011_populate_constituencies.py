...
def populate_constituencies(apps, schema_editor):
    ...
    data = {
        ...
        'Murang\'a':        ['Kandara', 'Gatanga', 'Kigumo', 'Kiharu', 'Kangema', 'Mathioya', 'Murang\'a South'],
    ...
    for county_name, constituencies in data.items():
        try:
            county = County.objects.get(name=county_name)
            for name in constituencies:
                Constituency.objects.get_or_create(name=name, county=county)
        except County.DoesNotExist:
            pass


def reverse_constituencies(apps, schema_editor):
    Constituency = apps.get_model('recruitment', 'Constituency')
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(populate_constituencies, reverse_constituencies),
    ]